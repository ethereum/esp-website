...
async function handler(req: SemaphoreGrantNextApiRequest, res: NextApiResponse) {
  ...
  conn.login(SF_PROD_USERNAME!, `${SF_PROD_PASSWORD}${SF_PROD_SECURITY_TOKEN}`, err => {
    ...
    const application = {
      ...
      Repeat_Applicant__c: Repeat_Applicant__c,
      Can_the_EF_reach_out__c: Can_the_EF_reach_out__c,
      Additional_Information__c: Additional_Information__c.trim(),

      Proactive_Community_Grants_Round__c: 'Semaphore Community Grants 2022', // this value is hardwired, depending on the type of grant round
